...
void lv_btnm_set_map(lv_obj_t * btnm, const char ** map)
{
    ...
    uint8_t line_cnt = 1;
    uint8_t li;
    for(li = 0; (size_t) AsciiStrLen(map[li]) != 0; li++) {
        if((int) AsciiStrCmp(map[li], "\n") == 0) line_cnt ++;
    }
    ...
    while(1) {
        ...
        while((int) AsciiStrCmp(map_p_tmp[btn_cnt], "\n") != 0 &&
                (size_t) AsciiStrLen(map_p_tmp[btn_cnt]) != 0) { /*Check a line*/
        ...
        if((size_t) AsciiStrLen(map_p_tmp[btn_cnt]) == 0) break; /*Break on end of map*/
...
static bool lv_btnm_design(lv_obj_t * btnm, const lv_area_t * mask, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        for(btn_i = 0; btn_i < ext->btn_cnt; btn_i ++, txt_i ++) {
            ...
            while((int) AsciiStrCmp(ext->map_p[txt_i], "\n") == 0) txt_i ++;
...
static void create_buttons(lv_obj_t * btnm, const char ** map)
{
    ...
    uint16_t btn_cnt = 0;
    uint16_t i = 0;
    while((size_t) AsciiStrLen(map[i]) != 0) {
        if((int) AsciiStrCmp(map[i], "\n") != 0) { /*Do not count line breaks*/
            btn_cnt ++;
        }
        i++;
    }
...
static uint16_t get_button_text(lv_obj_t * btnm, uint16_t btn_id)
{
    ...
    while(btn_i != btn_id) {
        btn_i ++;
        txt_i ++;
        if((int) AsciiStrCmp(ext->map_p[txt_i], "\n") == 0) txt_i ++;
    }
