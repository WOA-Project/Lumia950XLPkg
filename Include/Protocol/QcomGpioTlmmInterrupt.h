...
#define QCOM_GPIO_TLMM_INTERRUPT_PROTOCOL_GUID                                 \
  {                                                                            \
    0x1634c987, 0x50a7, 0x4f98,                                                \
    {                                                                          \
      0x88, 0xf0, 0x7e, 0xbc, 0x60, 0x11, 0xa5, 0x32                           \
    }                                                                          \
  }

typedef struct _QCOM_GPIO_TLMM_INTERRUPT_PROTOCOL
    QCOM_GPIO_TLMM_INTERRUPT_PROTOCOL;
...
#define TLMM_IRQ_TYPE_EDGE_BOTH                                                \
  (TLMM_IRQ_TYPE_EDGE_FALLING | TLMM_IRQ_TYPE_EDGE_RISING)
#define TLMM_IRQ_TYPE_LEVEL_HIGH 0x00000004
#define TLMM_IRQ_TYPE_LEVEL_LOW 0x00000008
#define TLMM_IRQ_TYPE_LEVEL_MASK                                               \
  (TLMM_IRQ_TYPE_LEVEL_LOW | TLMM_IRQ_TYPE_LEVEL_HIGH)
...
typedef VOID(EFIAPI *gpio_tlmm_irq_set_custom_handler_t)(
    UINTN id, GPIO_INTERRUPT_HANDLER handler);
