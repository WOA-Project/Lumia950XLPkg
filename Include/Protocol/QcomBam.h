...
#define QCOM_BAM_PROTOCOL_GUID                                                 \
  {                                                                            \
    0xacdd545a, 0xf1f6, 0x4272,                                                \
    {                                                                          \
      0x81, 0xc5, 0x04, 0x93, 0xe3, 0x58, 0x05, 0x32                           \
    }                                                                          \
  }
...
typedef void(EFIAPI *bam_sys_pipe_init_t)(
    struct bam_instance *bam, uint8_t pipe_num);
typedef int(EFIAPI *bam_pipe_fifo_init_t)(
    struct bam_instance *bam, uint8_t pipe_num);
typedef struct cmd_element *(EFIAPI *bam_add_cmd_element_t)(
    struct cmd_element *ptr, uint32_t addr, uint32_t data,
    enum bam_ce_cmd_t cmd_type);
typedef int(EFIAPI *bam_add_desc_t)(
    struct bam_instance *bam, unsigned int pipe_num, unsigned char *data_ptr,
    unsigned int data_len, unsigned flags);
typedef int(EFIAPI *bam_add_one_desc_t)(
    struct bam_instance *bam, unsigned int pipe_num, unsigned char *,
    uint32_t len, uint8_t flags);
typedef void(EFIAPI *bam_sys_gen_event_t)(
    struct bam_instance *bam, uint8_t pipe_num, unsigned int num_desc);
typedef int(EFIAPI *bam_wait_for_interrupt_t)(
    struct bam_instance *bam, uint8_t pipe_num, enum p_int_type interrupt);
typedef void(EFIAPI *bam_read_offset_update_t)(
    struct bam_instance *bam, unsigned int pipe_num);
