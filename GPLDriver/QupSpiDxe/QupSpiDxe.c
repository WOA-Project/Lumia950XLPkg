           ...
           QupSpiDxeEntry(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  Status = gBS->LocateProtocol(&gQcomClockProtocolGuid, NULL, (VOID **) &mClockProtocol);
  if (EFI_ERROR(Status)) {
    goto exit;
  }
  ...
  DEBUG((EFI_D_INFO, "IN:block:%d, fifo:%d, OUT:block:%d, fifo:%d\n",
		 mController.in_blk_sz, mController.in_fifo_sz,
		 mController.out_blk_sz, mController.out_fifo_sz));
  ...
  /* Register IRQ */
  register_int_handler(mController.irq, (int_handler) qup_spi_interrupt, &mController);
