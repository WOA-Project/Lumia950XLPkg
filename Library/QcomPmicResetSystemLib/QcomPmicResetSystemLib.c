...
STATIC VOID rt_pm8994_reset_configure(uint8_t reset_type)
{
	...
	*/
	...
	rt_pm8xxx_reg_write(slave_id[0], PON_PS_HOLD_RESET_CTL2, BIT(S2_RESET_EN_BIT));
}
...
VOID
EFIAPI
LibRuntimeVirtualAddressChanged(
	IN EFI_EVENT        Event,
	IN VOID             *Context
)
{
	gRT->ConvertPointer(
		0,
		(VOID **) &pPonPsHoldAddressVirtual
	);

	gRT->ConvertPointer(
		0,
		(VOID **) &pResetReasonAddressVirtual
	);
