...
{
    uint32_t old_len = (size_t) AsciiStrLen(txt_buf);
    uint32_t ins_len = (size_t) AsciiStrLen(ins_txt);
...
void lv_txt_cut(char * txt, uint32_t pos, uint32_t len)
{
    ...
    uint32_t old_len = (size_t) AsciiStrLen(txt);
#if LV_TXT_UTF8 != 0
