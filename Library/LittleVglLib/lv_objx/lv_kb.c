...
static lv_res_t lv_kb_def_action(lv_obj_t * kb, const char * txt)
{
    ...
    if((int) AsciiStrCmp(txt, "abc") == 0) {
        lv_btnm_set_map(kb, kb_map_lc);
        return LV_RES_OK;
    } else if((int) AsciiStrCmp(txt, "ABC") == 0) {
        lv_btnm_set_map(kb, kb_map_uc);
        return LV_RES_OK;
    } else if((int) AsciiStrCmp(txt, "1#") == 0) {
        lv_btnm_set_map(kb, kb_map_spec);
        return LV_RES_OK;
    } else if((int) AsciiStrCmp(txt, SYMBOL_CLOSE) == 0) {
        ...
        return LV_RES_INV;
    } else if((int) AsciiStrCmp(txt, SYMBOL_OK) == 0) {
    ...
    if((int) AsciiStrCmp(txt, "Enter") == 0)lv_ta_add_char(ext->ta, '\n');
    else if((int) AsciiStrCmp(txt, SYMBOL_LEFT) == 0) lv_ta_cursor_left(ext->ta);
    else if((int) AsciiStrCmp(txt, SYMBOL_RIGHT) == 0) lv_ta_cursor_right(ext->ta);
    else if((int) AsciiStrCmp(txt, "Del") == 0)  lv_ta_del_char(ext->ta);
    else if((int) AsciiStrCmp(txt, "+/-") == 0) {
