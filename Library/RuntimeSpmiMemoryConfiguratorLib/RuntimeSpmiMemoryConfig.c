...
)
{
	...
	Status = gDS->GetMemorySpaceDescriptor(
		ROUND_TO_PAGE(MPM2_MPM_PS_HOLD),
	...
	Status = gDS->GetMemorySpaceDescriptor(
		ROUND_TO_PAGE(MSM_SHARED_IMEM_BASE),
		&imemRegionMemoryDescriptor
	);
	...
	if (!(spmiMemoryDescriptor.Attributes & EFI_MEMORY_RUNTIME))
	{
		Status = gDS->SetMemorySpaceAttributes(
			...
			0x10000,
			spmiMemoryDescriptor.Attributes | EFI_MEMORY_RUNTIME
		);
