...
)
{
	...
	discard = snprintf(clk_name, sizeof(clk_name), "qup%08x_ahb_iface_clk", QupBase);
	...
	discard = snprintf(clk_name, sizeof(clk_name), "gcc_qup%08x_i2c_apps_clk", QupBase);
...
)
{
	...
	while (I2cDevice->DeviceId > 0)
	{
		...
		if (EFI_ERROR(Status)) {
			DEBUG((EFI_D_ERROR, "Can't configure clocks for I2C device %u \n", I2cDevice->DeviceId));
			goto nextDevice;
		}
		...
		if (EFI_ERROR(Status)) 
		{
			DEBUG((EFI_D_ERROR, "Failed register I2C device 0x%08x with ID %u\n", dev->qup_base, dev->id));
			goto nextDevice;
		}
