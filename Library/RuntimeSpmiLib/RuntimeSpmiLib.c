...
UINTN
EFIAPI
RtSpmiPmicArbWriteCmd(
	PRUNTIME_PMIC_ARB_CMD cmd,
	PRUNTIME_PMIC_ARB_PARAM param
)
{
	...
	RtSpmiWriteWDataFromArray(param->buffer, 0, param->size, (uint8_t *)&bytes_written);

	if (bytes_written < param->size)
	{
		/* Write next 4 bytes to WDATA1 */
		RtSpmiWriteWDataFromArray(param->buffer, 1, param->size, (uint8_t *)&bytes_written);
	}
