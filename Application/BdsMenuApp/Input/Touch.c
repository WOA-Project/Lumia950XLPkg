...
{
	if (mPointer == NULL || data == NULL) goto exit;
	...
	// Not changed
	if (State.CurrentX == LastX && State.CurrentY == LastY) goto exit;

	// Changed
	data->point.x = ((double)State.CurrentX / (double)mPointer->Mode->AbsoluteMaxX) * LV_HOR_RES;
	data->point.y = ((double)State.CurrentY / (double)mPointer->Mode->AbsoluteMaxY) * LV_VER_RES;
