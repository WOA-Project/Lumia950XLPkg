...
EFIAPI EFI_STATUS SystemInfoEntry(lv_obj_t* tbSysInfo)
{
	...
	UnicodeSPrint(TempStr, sizeof(TempStr), L"Firmware release: %s %d.%02d \n",
		gST->FirmwareVendor,
	...
	AsciiSPrint(TempStr8, sizeof(TempStr8), "Git commit: %a (%a) \n", __IMPL_COMMIT_ID__, __RELEASE_DATE__);
	create_label_with_text(tbSysInfo, TempStr8);

	AsciiSPrint(TempStr8, sizeof(TempStr8), "UEFI spec: %d.%02d \n",
		gST->Hdr.Revision >> 16,
	...
	AsciiSPrint(TempStr8, sizeof(TempStr8), "Processor: %a \n", (CHAR8*) FixedPcdGetPtr(PcdSmbiosProcessorModel));
	create_label_with_text(tbSysInfo, TempStr8);

	AsciiSPrint(TempStr8, sizeof(TempStr8), "System model: %a \n", (CHAR8*) FixedPcdGetPtr(PcdSmbiosSystemModel));
	create_label_with_text(tbSysInfo, TempStr8);
