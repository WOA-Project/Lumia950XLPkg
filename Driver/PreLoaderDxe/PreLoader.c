...
STATIC EFI_GUID gUnsupportedImageGuid = { 0x5fdf5e3c, 0x6b70, 0x4acc, { 0x83, 0x30, 0x63, 0xa6, 0x73, 0x92, 0x4a, 0x46 } };
...
static void EfiGopBltFlush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t* color_p)
{
	mGop->Blt(
		mGop,
...
static bool FakeInputRead(lv_indev_data_t* data)
{
	return false;
}

void ConvertBetweenBGRAandRGBA(uint8_t* input, int pixel_width, int pixel_height)
{
	int offset = 0;
...
)
{
	...
	Status = GetSectionFromAnyFv(&gUnsupportedImageGuid, EFI_SECTION_RAW, 0, &ImageData, &ImageSize);
	...
	// Decode PNG
	DecoderError = lodepng_decode32(&DecodedData, &Width, &Height, (unsigned char*) ImageData, (size_t) ImageSize);
	if (!DecoderError)
	{
...
)
{
	...
	// Version requirement
	if (Env->Header != PRELOADER_HEADER || Env->PreloaderVersion < PRELOADER_VERSION_MIN)
	{
		BootShimVersionCheckFail();
